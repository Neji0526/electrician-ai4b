...
```bash
npm install
npm run gen:placeholders   # writes the placeholder imagery into public/ (already committed)
...
| `npm run gen:placeholders` | Regenerates the SVG placeholder imagery |
...
`npm run build && npm run smoke` imports the built server handler and renders all 59 URLs
...
scripts/
  gen-placeholders.mjs   Generates the SVG placeholder imagery
...
| `/sitemap` · `/privacy` · `/terms` | HTML sitemap and legal pages |
...
`public/images/**` currently holds generated SVG placeholders — one per service, per project
(before *and* after), per article and per team member, each labelled with what belongs there.
They exist so layout, aspect ratios and alt text are real and testable before photography
arrives.

To replace them: drop a JPG/WebP at the same path and update the `src` in the matching
`src/content/*.ts` record. Alt text is already written for every image and describes the
intended photograph, not the placeholder.
